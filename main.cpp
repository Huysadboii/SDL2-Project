...
int main(int argc, char* argv[])
{
    ...
    string alive = "Still alive after falling";
    ...
    Text menu_start;
    menu_start.SetColor(Text::WHITE_TEXT);
    string menu_start_text = "Press Enter to start the game";
    Text menu_quit;
    menu_quit.SetColor(Text::WHITE_TEXT);
    string menu_quit_text = "Press Esc to quit the game";

    Text over_start;
    over_start.SetColor(Text::WHITE_TEXT);
    string over_start_text = "Press Enter to return to menu";
    Text over_quit;
    over_quit.SetColor(Text::WHITE_TEXT);
    string over_quit_text = "Press Esc to quit the game";
    Text over_score;
    over_score.SetColor(Text::WHITE_TEXT);
    string over_score_text;
    ...
    while(running)
    {
        ...
        switch ( currentState )
        {
            case Menu::MENU:
                ...
                menu_start.SetText(menu_start_text);
                menu_start.LoadFromRenderText(font_time, g_screen);
                menu_start.RenderText(g_screen, POS_OF_MENU, MENU_HEIGHT_1);

                menu_quit.SetText(menu_quit_text);
                menu_quit.LoadFromRenderText(font_time, g_screen);
                menu_quit.RenderText(g_screen, POS_OF_MENU, MENU_HEIGHT_2);
            ...
            case Menu::GAME_OVER:
                ...
                over_score_text = "YOUR SCORE: " + to_string(score_value);
                ...
                over_start.SetText(over_start_text);
                over_start.LoadFromRenderText(font_time, g_screen);
                over_start.RenderText(g_screen, POS_OF_MENU, MENU_HEIGHT_1);

                over_quit.SetText(over_quit_text);
                over_quit.LoadFromRenderText(font_time, g_screen);
                over_quit.RenderText(g_screen, POS_OF_MENU, MENU_HEIGHT_2);
            ...
            case Menu::VICTORY:
                ...
                over_score_text = "YOU WIN WITH THE SCORE: " + to_string(score_value);
                over_score.SetText(over_score_text);
                over_score.LoadFromRenderText(font_time, g_screen);
                over_score.RenderText(g_screen, OVER_SCORE_POS - 100, SCORE_HEIGHT);
            ...
            case Menu::PLAYING:
                ...
                while(!is_quit){
                    ...
                    if(p_player.CheckFall(map_data)){
                        if(!is_added) Mix_PlayChannel(-1, g_sound_exp_player, 0);
                        num_eliminated++;
                    ...
                    if(!is_added) display_coin.Show(g_screen);
                    ...
                    for(int i = 0; i < enemies.size(); i++){
                        ...
                        if(p_threat != NULL){
                            ...
                            bool is_falling = p_player.CheckFall(map_data);
                            if(bCol2 || bCol1){
                    ...
                    if(coin_count_val >= COIN_PER_LIFE && !is_added){
                        is_added = true;
                        num_eliminated--;
                        life_added++;
                    ...
                    coin_count.SetText(is_added ? alive : str_coin_count);
                    coin_count.LoadFromRenderText(font_time, g_screen);
                    coin_count.RenderText(g_screen, is_added ? POS_OF_TEXT : SCREEN_WIDTH*0.5 - 250, TEXT_FONT);
